...
<link href="css/style.css" rel="stylesheet" type="text/css" />
  ...
  <tr>
      ...
      <td  style="font: normal 450% 'Jenna Sue', arial, sans-serif;"><a href="javascript:LoadMonth('<?php echo $next_month; ?>', '<?php echo $next_year; ?>')">&gt;&gt;</a></td>
  </tr>
  <tr>
      <td class="wDays">M</td>
      <td class="wDays">T</td>
...
for ($i=1; $i<($maxday+$startday); $i++) {
	...
	if($date>$current_day && date("m")>=$cMonth && date("Y")>=$cYear){$css="booked";}
	echo "<td class='".$css."'	>";
	//echo $date;
...
//	if(date_diff($d2,$d1)>0){echo "<h1>hello</h1>";}
	$query=mysql_query("SELECT * from calendar WHERE date='$day'");
	if(!$query){die("sorry");}
	if(mysql_num_rows($query)==0 && $css!="booked"){
	if(isset($_SESSION['ann']) && $_SESSION['ann']==1){
	echo "<a href=\"speech2.php?date=".$code."\">". $current_day . "</a></td>";
	}
	else{
	echo "<a href=\"upload.php?date=".$code."\">". $current_day . "</a></td>";}
	}
