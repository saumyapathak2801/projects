...
public function vote() {
    ...
    if($this->data[$ID]) {
		...
		$no=$this->data[$ID]['number_votes'];
        $this->data[$ID]['total_points'] += $vote;
		$points=$this->data[$ID]['total_points'];
		$query=mysql_query("update songs set rating=$points, votes=$no where id=$ID");
		if(!$query)
		{die(mysql_error());}
	...
	$points=$this->data[$ID]['whole_avg'];
	$query=mysql_query("update songs set avg=$points where id=$ID");
		if(!$query)
		{die(mysql_error());}
