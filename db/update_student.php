...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM b");
			$myQuery = "show columns from student";
...
<?php
if(!empty($_POST['name']) && !empty($_POST['parents_name']) && !empty($_POST['rolln']) && !empty($_POST['dob']) && !empty($_POST['sex']) && !empty($_POST['contact']) && !empty($_POST['address']) && !empty($_POST['batch']) && !empty($_POST['status']) && isset($_POST['submit']))
{
				$query=mysql_query("update student set name='$_POST[name]', parents_name='$_POST[parents_name]', address='$_POST[address]', batch='$_POST[batch]', dob='$_POST[dob]', sex='$_POST[sex]', contact='$_POST[contact]', status='$_POST[status]' where roll='$_POST[rolln]'");
				if(!$query)
				{die("error".mysql_error());}
				else{header("Location:view.php?id=student");}
			}
	?>		
</html>
