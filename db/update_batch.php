...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM batch");
			$myQuery = "show columns from batch";
...
<?php
if(!empty($_POST['batch']) && !empty($_POST['prog_code']) && isset($_POST['submit']))
{
				$query=mysql_query("update batch set prog_code='$_POST[prog_code]' where batch_name='$_POST[batch]'");
				if(!$query)
				{die("error");}
				else{header("Location:view.php?id=batch");}
			}
	?>		
</html>
