...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM b");
			$myQuery = "show columns from employee";
...
<?php
if(!empty($_POST['address']) && !empty($_POST['salary']) && !empty($_POST['dept_no']) && !empty($_POST['dob']) && !empty($_POST['sex']) && !empty($_POST['contact']) && !empty($_POST['joining_date']) && !empty($_POST['name']) && isset($_POST['submit']))
{
				$query=mysql_query("update employee set name='$_POST[name]', address='$_POST[address]', salary='$_POST[salary]', dept_no='$_POST[dept_no]', dob='$_POST[dob]', sex='$_POST[sex]', contact='$_POST[contact]', joining_date='$_POST[joining_date]' where id='$_POST[emp_id]'");
				if(!$query)
				{die("error".mysql_error());}
				else{header("Location:view.php?id=employee");}
			}
	?>		
</html>
