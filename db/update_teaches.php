...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM b");
			$myQuery = "show columns from teaches";
...
<?php
if(!empty($_POST['id']) && !empty($_POST['batch']) && isset($_POST['submit']))
{
				$query=mysql_query("update teaches set batch='$_POST[batch]' where emp_id='$_POST[id]'");
				if(!$query)
				{die("error");}
				else{header("Location:view.php?id=teaches");}
			}
	?>		
</html>
