...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM b");
			$myQuery = "show columns from dept_wf";
	...
	//			$col=mysql_num_fields($row);
			...
			{
					echo "<select name=\"wf\">";
			$res=mysql_query("select id from dept_wf");
...
<?php
if(!empty($_POST['wf']) && !empty($_POST['workforce']) && isset($_POST['submit']))
{
				$query=mysql_query("update dept_wf set workforce='$_POST[workforce]' where id='$_POST[wf]'");
				if(!$query)
				{die("error");}
				else{header("Location:view.php?id=dept_wf");}
			}
	?>		
</html>
