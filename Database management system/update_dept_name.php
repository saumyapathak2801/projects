...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM b");
			$myQuery = "show columns from dept_name";
...
<?php
if(!empty($_POST['dept_name']) && !empty($_POST['name']) && isset($_POST['submit']))
{
				$query=mysql_query("update dept_name set name='$_POST[name]' where id='$_POST[dept_name]'");
				if(!$query)
				{die("error");}
				else{header("Location:view.php?id=dept_name");}
			}
	?>		
</html>
