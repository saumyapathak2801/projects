...
<div id="wrapper">
	...
	//		$res=mysql_query("SELECT * FROM b");
			$myQuery = "show columns from result";
	...
	//			$col=mysql_num_fields($row);
			...
			$res=mysql_query("select roll from result");
			while($row=mysql_fetch_row($res)){echo "<option value=\"$row[0]\">".$row[0]."</option>";};
			echo "</select></br></br>";
			}
			else if($row[0]=="test_id")
			{
			echo "<select name=\"id\">";
			$res=mysql_query("select test_id from result");
			while($row=mysql_fetch_row($res)){echo "<option value=\"$row[0]\">".$row[0]."</option>";};
			echo "</select>";
			}
			else{
...
<?php
if(!empty($_POST['roll']) && !empty($_POST['id']) && !empty($_POST['marks']) && !empty($_POST['percentile']) && isset($_POST['submit']))
{
				$query=mysql_query("update result set marks='$_POST[marks]',percentile='$_POST[percentile]' where roll='$_POST[roll]' and test_id='$_POST[id]' ");
				if(!$query)
				{die("error");}
				else{header("Location:view.php?id=result");}
			}
	?>		
</html>
