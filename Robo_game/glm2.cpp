...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	{
		//printf("%s %f %f %f\n",model->materials[i].name,model->materials[i].diffuse[0],model->materials[i].diffuse[1],model->materials[i].diffuse[2]);
	}
}
...
glmDraw(GLMmodel* model, GLuint mode)
{
	...
	while (group) 
	{
		...
		{
			glDisable(GL_COLOR_MATERIAL);
			float shininess = 25,  emissivity = 0.05f, specularity = 0.3f;
