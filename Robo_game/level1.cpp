...
void conv();
void obj1();
int finish=0,timer=0,daytimer=0;
int flagontop=0;
float str_x=0.5;
float coinrot=0.0f;
...
float minh=0.0f;
float ydec=0,w,hi;
int flag=0,flags=1,flagx=1,flagsc=1,flagr=1,fobj1=0,fstair=0,fobj2=0,flagf=0;
int flook=0;
...
float tri_y = 0.0f;
float theta = 0.0f; 
int seltile=0,count=0;
float lookx=0.0f,looky=0.0f,lookz=5.0f;
float strip_z=2.0f,strip_x=-2.5,rot_y=45,rots_z=4.05,fal_y=-.2;
...
const char *STRS[3] = {"Game Over", "C ongrats\nYou Win!", "Rock"};
...
//if(flaggaze!=1)
	    ...
	    if(flag2==0)
	    {
		    ...
		    if(third==1)
		    {
		   	towerx=xcoor;towery=ycoor+0.6f;towerz=zcoor-1.5f;
	    ...
	    if(flag2==1)
	    {
		    ...
		    if(third==1)
		    {towerx=zcoor-1.5f;towery=ycoor+0.6f;towerz=-xcoor;
...
//    }
    ...
    //  	centerx=xcoor;centery=ycoor;centerz=zcoor;
	   // }	
float ltx=1.0,lty=5.0,ltz=6.0;
glEnable(GL_LIGHT2);
float ambient1[] = {1, 1, 0, 1};
float position1[] = {ltx,lty+10,ltz, 1};
float direction1[] = {lookx-ltx,looky-lty-10,lookz-8.5-ltz};
...
if(drop==0 && health!=0 &&  flagontop==1)
{
	if(ycoor>-0.2f)
	{health=health-0.2f;}
	ycoor=-0.2f;
...
glShadeModel(GL_SMOOTH);
    ...
    delete image2;
    Image* image3 = loadBMP("stars.bmp");
...
void drawWorld2() // 7*7 world in -x
{
	...
	{    
	 ...
	 sz=3.2;
	   ...
	   {
	   if((i==5 && j==6))
	   {if(lookx>sx-.25 && lookx<sx+.4 && lookz<sz && lookz>sz-.4)
	   {flagf=1;
	   glPushMatrix();
	   glTranslatef(sx,fal_y,sz);
	   glScalef(1,.1,1);
	  ...
	  health=0;
	   ...
	   if((i==3 && j==3))
	   {if(lookx>sx-.25 && lookx<sx+.3 && lookz<sz+.25 && lookz>sz-.25)
	   {
	   flagf=1;
	   glPushMatrix();
	   glTranslatef(sx,fal_y,sz);
...
//	glmDraw(mod1, GLM_SMOOTH | GLM_TEXTURE | GLM_COLOR);
	glPopMatrix();
	glPopMatrix();
if(lookx<0.44+0.2 && lookx>0.44-0.2 && lookz>-1.8-0.2 && lookz < -1.8+0.2)
{obj[0].y=-2;obj[0].type=1;}
...
{ count=0;
	printf("epic %f %f\n",lookx,lookz);
	if(lookx>-2.42-.25 && lookx<-2.42+.2 && jump==0)//moving tile in -x 
	...
	if(lookx > 3.75-.8 && lookx<3.75+.9 )//moving tile in +x direc
	...
	{drop=1;health=0;
					 glutTimerFunc(100, update2, 0);
	ycoor=ydec;}
	if(lookz<.68-.4 && lookx<-5.36+.25)
	{drop=1;health=0;
					 glutTimerFunc(100, update2, 0);
	ycoor=ydec;}

if(lookx < obj[1].x+.2 && lookx> obj[1].x-.2 && lookz > obj[1].z-.2 && lookz< obj[1].z+.2)//obj1 collection
{fobj1=1;}

if(lookx < obj[2].x+1.2+.2 && lookx> obj[2].x+1.2-.2 && lookz > obj[2].z-.1-.4 && lookz< obj[2].z-.1+.2)//obj2 collection
{fobj2=1;}
...
if(lookz<-0.12-0.4)
 ...
 if(lookz<-0.12 && lookz>-0.12-0.4)
        {
                printf("far\n");
                if((lookx < (moving[1].x-0.2)) || (lookx > (moving[1].x+0.2)))
                {
                        printf("fall %f %f movx:%f movz:%f\n",lookx,lookz,moving[1].x,moving[1].z);
                        drop=1;health=0;
					 glutTimerFunc(100, update2, 0);
                        ycoor=ydec;
                }
...
/*	if(lookx>4.5-.25 && lookx<6.6-.2 && lookz<3.65 && lookz > 3-.65)
	...
	ycoor=-.2;}
*/
if(lookx>4.5-.25 && lookx<11)
{if(lookz<3-.6 || lookz>3+.6)
{drop=1;health=0;
					 glutTimerFunc(100, update2, 0);
ycoor=ydec;
}
...
if(lookx>-2-.25 && lookx<2.62+.25 && lookz>4.5)
{drop=1;health=0;
					 glutTimerFunc(100, update2, 0);
ycoor=ydec;}
...
void drawWorld5()//final destination(stairs)
{
float sx=4.5;
float sz=3;
for(int i=1;i<=5;i++)
...
if(fobj2==0)
	{glPushMatrix();
	glTranslatef(obj[2].x,obj[2].y,obj[2].z);
	GLUquadricObj *quadratic;
	quadratic = gluNewQuadric();
	 glColor3f(.2,1,.2);
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
	gluCylinder(quadratic,0.0f,0.2f,0.6f,32,32);
	glPopMatrix();
}
else
{obj[2].type=1;}
