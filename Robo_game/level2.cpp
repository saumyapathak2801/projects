...
void conv();
void obj1();
int finish=0,count=0,daytimer=0;
float str_x=0.5;
float coinrot=0.0f;
float xcoor=0.0f,ycoor=-0.2f,zcoor=3.0f,centerx=0.0f,centery=0.0f,centerz=3.0f;
float ang=0;
float minh=0.0f;
float ydec=0,w,hi;
int flag=0,flags=1,flagx=1,flagsc=1,flagr=1,fobj1=0,fobj2=0;
int flook=0;
...
//if(flaggaze!=1)
	    ...
	    if(flag2==0)
	    {
		    ...
		    if(third==1)
		    {
		   	towerx=xcoor;towery=ycoor+0.6f;towerz=zcoor-2.5f;
	    ...
	    if(flag2==1)
	    {
		    ...
		    if(third==1)
		    {towerx=zcoor-2.5f;towery=ycoor+0.6f;towerz=-xcoor;
...
glBindTexture(GL_TEXTURE_2D,_textureId5);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		 ...
		 glVertex3f(150.2,0.0,50.2);
		 glTexCoord2f(1.0f, 1.0f);
		 glVertex3f(150.2,0.0,-250.2);
		 glTexCoord2f(0.0f, 1.0f);
		 glVertex3f(-150.2,0.0,-250.2);
		 glEnd();
		 glPopMatrix();
		 moving[0].x=-2.42;moving[0].y=-0.2;moving[0].z=strip_z;
		 moving[1].x=str_x;moving[1].y=-0.2;moving[1].z=-0.12;
		 moving[2].x=3.0;moving[2].y=-0.2;moving[2].z=strip_z;
		 moving[3].x=str_x;moving[3].y=-0.2;moving[3].z=4.92;
		 printf("strip_z:%f\n",strip_z);
...
strcpy(score,"score:");
strcat(score,cou);
const char *STRS[3] = {score, "Congrats\nYou Win!", "Rock"};
...
glShadeModel(GL_SMOOTH);
    ...
    delete image;
    Image* image2 = loadBMP("sand.bmp");
    _textureId2 = loadTexture(image2);
    delete image2;
    Image* image3 = loadBMP("soil.bmp");
    _textureId3 = loadTexture(image3);
    delete image3;
    Image* image4 = loadBMP("mount.bmp");
    _textureId4 = loadTexture(image4);
    delete image4;
    Image* image5 = loadBMP("fire.bmp");
...
//		r=rand()%10;
		...
		{
			if(lookx<sx+0.25 && lookx>sx-0.25 && lookz<sz-0.2+0.25 && lookz>sz-0.2-0.25)
			{
				drop=1;
				health=0;
				ycoor=ydec;
...
void conv()
{
	...
	if(flag2==0)
	{
ycoor=looky,zcoor=lookz;xcoor=lookx;
}
	if(flag2==1)
	{
...
for(i=1;i<=10;i++)
{
	...
	{
	glPushMatrix();
	glTranslatef(sx+0.1,-0.2,sz-0.8);
	glScalef(2,str_x,0.1);
	drawcube(_textureId);
	glPopMatrix();
	moving[0].x=sx+0.1;moving[0].y=-0.2;moving[0].z=sz-0.8;
	}
	sz-=0.85;
}
...
for(i=1;i<=10;i++)
{
	...
	{
	glPushMatrix();
	glTranslatef(sx2+0.1,-0.2,sz-0.5);
	glScalef(0.1,str_x,2);
	drawcube(_textureId);
	glPopMatrix();
	moving[1].x=sx2+0.1;moving[1].y=-0.2;moving[1].z=sz-0.8;
	}
	sx2+=0.85;
}
sx1=sx1+.85;
...
for(i=1;i<=7;i++)
{
	...
	{
		glPushMatrix();
		moving[2].x=sx1;moving[2].y=-0.2;moving[2].z=sz-0.2;
		glTranslatef(sx1,-0.2,sz-0.2);
		glScalef(0.1,str_x,2);
		drawcube(_textureId);
		glPopMatrix();
	}
...
for(i=1;i<=7;i++)
{
	...
	{
	glPushMatrix();
	glTranslatef(sx2,-0.2,sz-0.1);
	glScalef(0.1,str_x,2);
	drawcube(_textureId);
	glPopMatrix();
	moving[3].x=sx2;moving[3].y=-0.2;moving[3].z=sz-0.1;
	}
	sx2-=.85;
}
...
glColor3f(float(165.0/255.0),float(42.0/255.0),float(42.0/255.0));
glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
gluCylinder(quadratic,0.0f,0.1f,0.5f,32,32);
glPopMatrix();
}
else
{obj[0].type=1;}
...
if(fobj2==0)
{glPushMatrix();
glTranslatef(obj[1].x,obj[1].y,obj[1].z);
quadratic = gluNewQuadric();
glColor3f(float(165.0/255.0),float(42.0/255.0),float(42.0/255.0));
glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
gluCylinder(quadratic,0.0f,0.12f,0.6f,32,32);
glPopMatrix();}
else
{obj[1].type=1;}
...
{
count=0;
if(lookx > -.45 && lookx<.45 && lookz<3+.45 && lookz> -5.5+.3)
{drop=0;}
else if(lookx<-.8+.45 && lookx>-8.5-.5 && lookz>-5.5+.3 && lookz<-5.5+1.25)
{drop=0;}
else if(lookx>.8-.45 && lookx<8.5+.6 && lookz>-5.5+.3 && lookz<-5.5+1.25)
{drop=0;}
else if(lookx >8.5-.45 && lookx<8.5+.6 && lookz >-5.5+.3 && lookz<3+.45)
{drop=0;}
else if(lookx >-8.5-.5 && lookx<-8.5+.45 && lookz >-5.5+.3 && lookz<3+.45)
{drop=0;}
else if(lookx<8.5+.6 && lookx > 2.55-.5 && lookz <3+.45 && lookz>3-.5)
{drop=0;}
else if(lookx>-8.5-.5 && lookx <-2.55+.5 && lookz<3+.45 && lookz>3-.5)
{drop=0;}
else if(lookx>2.55-.5 && lookx< 2.55+.5 && lookz <3+.45 && lookz>-1.25-.6)
{drop=0;}
else if(lookx<-2.55+.5 && lookx>-2.55-.5 && lookz <3+.45 && lookz>-1.25-.6)
{drop=0;}
else if(lookx<-2.55+.5 && lookx>-5.1-.6 && lookz <-1.25+.5 && lookz>-1.25-.6)
{drop=0;}
else if(lookx>2.55-.5 && lookx<5.1+.6 && lookz <-1.25+.5 && lookz>-1.25-.6)
{drop=0;}
else if(lookx>-5.1-.6 && lookx<-5.1+.5 && lookz>-1.25-.6 && lookz<.45+.5)
{drop=0;}
else if(lookx>5.1-.6 && lookx<5.1+.5 && lookz>-1.25-.6 && lookz<.45+.5)
{drop=0;}
else
{drop=1;
	health=0;
	ycoor=ydec;}
if(lookx<obj[0].x+.3 && lookx > obj[0].x-.2 && lookz<obj[0].z+.2 && lookz>obj[0].z-.2)// obj1 collect
{fobj1=1;}

if(lookx<obj[1].x+.2 && lookx > obj[1].x-.5 && lookz<obj[1].z+.2 && lookz>obj[1].z-.2)// obj2 collect
{fobj2=1;}
...
for(i=0;i<4;i++)
{
	printf("bounce lookx:%f movx:%f lookz:%f movz:%f str_x:%f\n",lookx,moving[i].x,lookz,moving[i].z,str_x);
	if(lookx<moving[i].x+0.4 && lookx>moving[i].x-0.4 && lookz<moving[i].z+0.4 && lookz>moving[i].z-0.4 && str_x>0)
	{
		zcoor-=0.5;
	}
}
